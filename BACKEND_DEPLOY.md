# 🚀 Деплой Backend API

## 📋 Варианты хостинга Backend

### 1. **Railway** (Рекомендуется - самый простой) ⭐
### 2. **Render** (Бесплатный тариф)
### 3. **Fly.io** (Хорошая альтернатива)
### 4. **AWS Elastic Beanstalk** (Для production)

---

## 🌟 Вариант 1: Railway (Самый простой)

### Преимущества:
- ✅ Бесплатный тариф ($5 кредитов в месяц)
- ✅ Автоматический деплой из Git
- ✅ Встроенная база данных (PostgreSQL)
- ✅ Простая настройка
- ✅ Автоматический HTTPS

### Шаги деплоя:

#### 1. Подготовка проекта

Убедитесь, что у вас есть:
- `Dockerfile.backend` ✅
- `requirements.txt` ✅
- `app/` папка с кодом ✅

#### 2. Деплой через Railway Dashboard

1. Зайдите на [railway.app](https://railway.app)
2. Нажмите **"Start a New Project"**
3. Выберите **"Deploy from GitHub repo"**
4. Выберите ваш репозиторий
5. Railway автоматически определит Dockerfile

#### 3. Настройка переменных окружения

В настройках проекта → **Variables** добавьте:

```env
# База данных (Railway создаст автоматически)
DATABASE_URL=${{Postgres.DATABASE_URL}}

# Или если используете внешнюю БД:
DB_URL=postgresql+psycopg://user:password@host:5432/cardb

# JWT настройки
SECRET_KEY=your-very-secure-secret-key-min-32-characters-long
ALGORITHM=HS256
ACCESS_TOKEN_EXPIRE_MINUTES=1440

# CORS (добавьте ваш Vercel URL)
CORS_ORIGINS=https://fullstack-lab-work.vercel.app,https://fullstack-lab-work-oajjvn4s2-llls-projects-d13c13b6.vercel.app
```

#### 4. Добавление PostgreSQL базы данных

1. В Railway Dashboard нажмите **"+ New"**
2. Выберите **"Database"** → **"Add PostgreSQL"**
3. Railway автоматически создаст переменную `DATABASE_URL`
4. Обновите переменные окружения, чтобы использовать `${{Postgres.DATABASE_URL}}`

#### 5. Обновление CORS в коде

Нужно обновить `app/main.py` чтобы добавить Vercel домены в CORS:

```python
# Получаем CORS origins из переменных окружения
cors_origins = os.getenv("CORS_ORIGINS", "http://localhost:3000").split(",")
cors_origins = [origin.strip() for origin in cors_origins]

app.add_middleware(
    CORSMiddleware,
    allow_origins=cors_origins,
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)
```

#### 6. Получение URL вашего API

После деплоя Railway даст вам URL вида:
```
https://your-project-name.up.railway.app
```

Скопируйте этот URL - он понадобится для настройки frontend!

---

## 🌐 Вариант 2: Render (Бесплатный)

### Преимущества:
- ✅ Полностью бесплатный тариф
- ✅ Автоматический деплой из Git
- ✅ Встроенная PostgreSQL (бесплатно)
- ✅ Автоматический HTTPS

### Шаги деплоя:

#### 1. Деплой через Render Dashboard

1. Зайдите на [render.com](https://render.com)
2. Нажмите **"New +"** → **"Web Service"**
3. Подключите ваш GitHub репозиторий
4. Настройки:
   - **Name:** `car-management-api`
   - **Environment:** `Docker`
   - **Dockerfile Path:** `Dockerfile.backend`
   - **Region:** Выберите ближайший

#### 2. Добавление PostgreSQL базы данных

1. Нажмите **"New +"** → **"PostgreSQL"**
2. Настройки:
   - **Name:** `car-db`
   - **Database:** `cardb`
   - **User:** `admin`
   - **Region:** Тот же, что и для Web Service
3. После создания скопируйте **Internal Database URL**

#### 3. Настройка переменных окружения

В настройках Web Service → **Environment**:

```env
DB_URL=<Internal Database URL из PostgreSQL>
SECRET_KEY=your-very-secure-secret-key-min-32-characters
ALGORITHM=HS256
ACCESS_TOKEN_EXPIRE_MINUTES=1440
CORS_ORIGINS=https://fullstack-lab-work.vercel.app
```

#### 4. Получение URL

Render даст вам URL вида:
```
https://car-management-api.onrender.com
```

---

## 🚁 Вариант 3: Fly.io

### Шаги деплоя:

```bash
# Установка Fly CLI
curl -L https://fly.io/install.sh | sh

# Вход в аккаунт
fly auth login

# Инициализация проекта
fly launch

# Создание PostgreSQL базы данных
fly postgres create

# Привязка базы данных
fly postgres attach <db-name>

# Деплой
fly deploy
```

---

## ⚙️ Обновление Frontend после деплоя Backend

После того как получите URL вашего backend API:

1. Зайдите в **Vercel Dashboard**
2. Выберите ваш проект
3. Перейдите в **Settings** → **Environment Variables**
4. Обновите `NEXT_PUBLIC_API_BASE_URL`:
   ```
   NEXT_PUBLIC_API_BASE_URL=https://your-backend-url.railway.app
   ```
5. Нажмите **"Redeploy"**

---

## 🔧 Обновление CORS в Backend

**ВАЖНО:** Нужно обновить `app/main.py` чтобы принимать запросы с Vercel:

```python
import os

# Получаем CORS origins из переменных окружения или используем дефолтные
cors_origins_env = os.getenv("CORS_ORIGINS", "")
if cors_origins_env:
    cors_origins = [origin.strip() for origin in cors_origins_env.split(",")]
else:
    # Дефолтные значения для разработки
    cors_origins = [
        "http://localhost:3000",
        "http://localhost:3001",
        "http://127.0.0.1:3000",
        "https://fullstack-lab-work.vercel.app",
        "https://fullstack-lab-work-oajjvn4s2-llls-projects-d13c13b6.vercel.app",
    ]

app.add_middleware(
    CORSMiddleware,
    allow_origins=cors_origins,
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)
```

---

## 📝 Чеклист деплоя

- [ ] Backend успешно деплоится
- [ ] PostgreSQL база данных создана и подключена
- [ ] Переменные окружения настроены
- [ ] CORS обновлен для Vercel доменов
- [ ] API доступен по HTTPS
- [ ] Frontend обновлен с новым API URL
- [ ] Тестирование: Frontend может подключиться к Backend

---

## 🐛 Решение проблем

### Проблема: Backend не может подключиться к БД

**Решение:**
1. Проверьте переменную `DATABASE_URL` или `DB_URL`
2. Убедитесь, что база данных запущена
3. Проверьте формат URL: `postgresql+psycopg://user:password@host:5432/dbname`

### Проблема: CORS ошибки

**Решение:**
1. Убедитесь, что Vercel URL добавлен в `CORS_ORIGINS`
2. Проверьте, что используется HTTPS (не HTTP)
3. Перезапустите backend после изменения CORS

### Проблема: 500 ошибки на API

**Решение:**
1. Проверьте логи в Railway/Render Dashboard
2. Убедитесь, что все переменные окружения установлены
3. Проверьте, что база данных инициализирована

---

## 🎯 Рекомендации

1. **Для быстрого деплоя:** Используйте Railway
2. **Для бесплатного хостинга:** Используйте Render
3. **Для production:** Используйте Railway или AWS

---

**Удачи с деплоем Backend! 🚀**

